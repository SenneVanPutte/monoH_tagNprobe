#include "TagAndProbe_Trigger/NtupleProducer/interface/Ntupler_monoH_16.h"
...
//
Ntupler_monoH_16::Ntupler_monoH_16(const edm::ParameterSet& iConfig):
  ...
  // 2016
  cutBasedIdTightSum16_token_(consumes<edm::ValueMap<bool> >(iConfig.getParameter<edm::InputTag>("cutBasedIdTightSum16"))),
  cutBasedIdHLT_token_(consumes<edm::ValueMap<bool> >(iConfig.getParameter<edm::InputTag>("cutBasedIdHLT"))),
...
//  tree_->Branch("isTrue"    , &isTrue_);
  ...
  // 2016
  tree_->Branch("passCutBasedIdTightSum16"  ,  &passCutBasedIdTightSum16_ );
  tree_->Branch("passCutBasedIdHLT"  ,  &passCutBasedIdHLT_ );
 ...
 // MonoHiggs
  tree_->Branch("passFilterDoubleEle33_MW_leg"           ,  &passFilterDoubleEle33_MW_leg);
  tree_->Branch("passFilterDoubleEle33_GsfTrkIdVL_leg"   ,  &passFilterDoubleEle33_GsfTrkIdVL_leg);
  tree_->Branch("passFilterDoubleEle33_GsfTrkIdVL_MW_leg",  &passFilterDoubleEle33_GsfTrkIdVL_MW_leg);
  tree_->Branch("passFilterMu27_Ele37_legEle"   ,  &passFilterMu27_Ele37_legEle);
  tree_->Branch("passFilterMu27_Ele37_emu_legEle"   ,  &passFilterMu27_Ele37_emu_legEle);
  tree_->Branch("passFilterMu37_Ele27_legEle"   ,  &passFilterMu37_Ele27_legEle);

  tree_->Branch("passFilterMu33_Ele33_legEle"   ,  &passFilterMu33_Ele33_legEle);
 ...
 // MonoHiggs
  ...
  tree_->Branch("passFilterMu30_TkMu11_leg1" ,  &passFilterMu30_TkMu11_leg1);
  tree_->Branch("passFilterMu30_TkMu11_leg2" ,  &passFilterMu30_TkMu11_leg2);
  tree_->Branch("passFilterMu27_Ele37_legMu"   ,  &passFilterMu27_Ele37_legMu);
  tree_->Branch("passFilterMu37_Ele27_legMu"   ,  &passFilterMu37_Ele27_legMu);

  tree_->Branch("passFilterMu33_Ele33_legMu"   ,  &passFilterMu33_Ele33_legMu);
  tree_->Branch("passFilterMu33_Ele33_emu_legMu"   ,  &passFilterMu33_Ele33_emu_legMu);
...
Ntupler_monoH_16::~Ntupler_monoH_16()
{
...
void
Ntupler_monoH_16::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace std;
  using namespace edm;
  using namespace reco;
  
  TString ele_filters[7] = {
  ...
  TString mu_filters[8] = {
    ...
    // MonoHiggs
    // MuEle legMu
    "hltL3fL1sMu16orMu25L1f0L2f10QL3Filtered27Q", //ok
    "hltL3fL1sMu16orMu25L1f0L2f10QL3Filtered37Q", //ok
    // Double Mu
    "hltL3fL1sMu16orMu25L1f0L2f25L3Filtered30", //ok
    "hltDiMuonGlbFiltered30TrkFiltered11", //ok
    // SingleMu
    "hltL3fL1sMu22Or25L1f0L2f10QL3Filtered50Q", //ok
    // Latino 2016 for Tag
...
//  }
  ...
  // 2016
  edm::Handle<edm::ValueMap<bool> > cutBasedIdTightSum16_handle;
  edm::Handle<edm::ValueMap<bool> > cutBasedIdHLT_handle;
  iEvent.getByToken(cutBasedIdTightSum16_token_, cutBasedIdTightSum16_handle);
  iEvent.getByToken(cutBasedIdHLT_token_, cutBasedIdHLT_handle);
...
//      ele_isTrue_.clear();
      ...
      // 2016
      passCutBasedIdTightSum16_.clear();
      passCutBasedIdHLT_.clear();
      ...
      // MonoHiggs
      passFilterDoubleEle33_MW_leg           .clear();
      passFilterDoubleEle33_GsfTrkIdVL_leg   .clear();
      passFilterDoubleEle33_GsfTrkIdVL_MW_leg.clear();
      passFilterMu27_Ele37_legEle.clear();
      passFilterMu27_Ele37_emu_legEle.clear();
      passFilterMu37_Ele27_legEle.clear();
      
      passFilterMu33_Ele33_legEle.clear();
      ...
      // MonoHiggs
      bool filterDoubleEle33_MW_leg = false;
      bool filterDoubleEle33_GsfTrkIdVL_leg = false;
      bool filterDoubleEle33_GsfTrkIdVL_MW_leg = false;
      bool filterMu27_Ele37_legEle = false;
      bool filterMu27_Ele37_emu_legEle = false;
      bool filterMu37_Ele27_legEle = false;

      bool filterMu33_Ele33_legEle = false;
...
//
        ...
        // MonoHiggs
        if(ele_filters[2].Contains(filter) && foundTheLeg)  filterDoubleEle33_MW_leg = true;
        if(ele_filters[3].Contains(filter) && foundTheLeg)  filterDoubleEle33_GsfTrkIdVL_leg = true;
        if(ele_filters[4].Contains(filter) && foundTheLeg)  filterDoubleEle33_GsfTrkIdVL_MW_leg = true;
        if(ele_filters[4].Contains(filter) && foundTheLeg && legObjects[iteTrigObj].at(iPass).pt()>=37)  filterMu27_Ele37_emu_legEle = true;
        //cout << "elefilter[9]=" << ele_filters[9] << ", filter=" << filter << ", contains=" << ele_filters[9].Contains(filter) << ", legfound=" << foundTheLeg << endl;
        if(ele_filters[0].Contains(filter) && foundTheLeg)  filterMu27_Ele37_legEle = true;
        if(ele_filters[1].Contains(filter) && foundTheLeg)  filterMu37_Ele27_legEle = true;

        if(ele_filters[6].Contains(filter) && foundTheLeg)  filterMu33_Ele33_legEle = true;

        if(ele_filters[5].Contains(filter) && foundTheLeg)  filterEle27 = true;
      }
      ...
      // MonoHiggs 
      passFilterDoubleEle33_MW_leg           .push_back(filterDoubleEle33_MW_leg);
      passFilterDoubleEle33_GsfTrkIdVL_leg   .push_back(filterDoubleEle33_GsfTrkIdVL_leg);
      passFilterDoubleEle33_GsfTrkIdVL_MW_leg.push_back(filterDoubleEle33_GsfTrkIdVL_MW_leg);
      passFilterMu27_Ele37_legEle  .push_back(filterMu27_Ele37_legEle);
      passFilterMu27_Ele37_emu_legEle  .push_back(filterMu27_Ele37_emu_legEle);
      passFilterMu37_Ele27_legEle  .push_back(filterMu37_Ele27_legEle);

      passFilterMu33_Ele33_legEle  .push_back(filterMu33_Ele33_legEle);
...
//        printf("Electron energy is zero!\n");
    ...
    // 2016 
    bool isPassCutBasedIdTightSum16  = (*cutBasedIdTightSum16_handle)[el];
    bool isPassCutBasedIdHLT  = (*cutBasedIdHLT_handle)[el];
    ...
    // 2016
    passCutBasedIdTightSum16_.push_back  ( (int)isPassCutBasedIdTightSum16  );
    passCutBasedIdHLT_.push_back  ( (int)isPassCutBasedIdHLT  );
...
//  if(isAOD) iEvent.getByToken(muonsToken_,muons);
 ...
 // Handle over L1-muon
  ...
  if(doMuon_)
  {
     ...
     // MonoHiggs
     passFilterIsoMu24.clear(); //yes you also need to clear this to not get branches of 95 GiB
     passFilterMu50          .clear();
     passFilterMu30_TkMu11_leg1 .clear();
     passFilterMu30_TkMu11_leg2 .clear();
     passFilterMu27_Ele37_legMu.clear();
     passFilterMu37_Ele27_legMu.clear();

     passFilterMu33_Ele33_legMu.clear();
     passFilterMu33_Ele33_emu_legMu.clear();
     ...
     for(unsigned i=0; i < muons->size();++i ) {
       ...
       float temp3 = 9999.0;
       float delRL1_7=9999;
       float delRL1_16=9999;
       float delRL1_23=9999;
       float delRL1_7_toBeChecked=9999;
       float delRL1_16_toBeChecked=9999;
       ...
       if (L1muons.isValid()) {
        for(int i=L1muons->getFirstBX(); i<=L1muons->getLastBX();i++) {
          for(std::vector<l1t::Muon>::const_iterator L1mu = L1muons->begin(i); L1mu != L1muons->end(i); ++L1mu) {
                ...
                if((L1MuPt >= 16 || L1MuPt >= 25) && (L1MuQual>=12 && L1MuQual<=15))
                {
                   delRL1_16 = deltaR(L1MuEta,L1MuPhi ,mu->eta(),mu->phi());
                   if (delRL1_16<temp2) temp2 = delRL1_16;
                }
                ...
                if(temp2<0.3) delRL1_16_toBeChecked=temp2;
       ...
       bool filterMu30_TkMu11_Leg1 = false;
       bool filterMu30_TkMu11_Leg2 = false;
       bool filterMu27_Ele37_legMu = false;
       bool filterMu37_Ele27_legMu = false;

       bool filterMu33_Ele33_legMu = false;
       bool filterMu33_Ele33_emu_legMu = false;
      ...
      // Trigger matching
       for (unsigned int iteTrigObj = 0 ; iteTrigObj < filterToMatch_.size() ; iteTrigObj++){
          ...
          // MonoHiggs
          if(mu_filters[4].Contains(filter) && foundTheLeg)  filterMu50 = true;
          if(mu_filters[2].Contains(filter) && foundTheLeg)  {filterMu30_TkMu11_Leg1 = true; }
          if(mu_filters[3].Contains(filter) && foundTheLeg)  {filterMu30_TkMu11_Leg2 = true; }
          if(mu_filters[0].Contains(filter) && foundTheLeg)  {filterMu27_Ele37_legMu = true; }
          if(mu_filters[1].Contains(filter) && foundTheLeg) {filterMu37_Ele27_legMu = true; }

          if(mu_filters[6].Contains(filter) && foundTheLeg) {filterMu33_Ele33_legMu = true; }
          if(mu_filters[7].Contains(filter) && foundTheLeg && legObjects[iteTrigObj].at(iPass).pt()>=33) {filterMu33_Ele33_emu_legMu = true; }
     
          if(mu_filters[5].Contains(filter) && foundTheLeg)  filterIsoMu24 = true;
       }
       ...
       passFilterMu30_TkMu11_leg1 .push_back(filterMu30_TkMu11_Leg1);
       passFilterMu30_TkMu11_leg2 .push_back(filterMu30_TkMu11_Leg2);
       passFilterMu27_Ele37_legMu .push_back(filterMu27_Ele37_legMu);
       passFilterMu37_Ele27_legMu .push_back(filterMu37_Ele27_legMu);

       passFilterMu33_Ele33_legMu .push_back(filterMu33_Ele33_legMu);
       passFilterMu33_Ele33_emu_legMu .push_back(filterMu33_Ele33_emu_legMu);
...
void 
Ntupler_monoH_16::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
Ntupler_monoH_16::endJob() 
{
}
...
/*
void 
Ntupler_monoH_16::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
Ntupler_monoH_16::endRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
Ntupler_monoH_16::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
Ntupler_monoH_16::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
void
Ntupler_monoH_16::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
int Ntupler_monoH_16::matchToTruth(const edm::Ptr<reco::GsfElectron> el, 
  ...
  if( ancestorPID == -999 && ancestorStatus == -999 ){
    ...
    // Complain.
    printf("Ntupler_monoH_16: ERROR! Electron does not apper to have a non-electron parent\n");
    return UNMATCHED;
  }
...
bool Ntupler_monoH_16::cmd(const reco::GenParticle & s1, const reco::GenParticle & s2)
{  
   return s1.pt() > s2.pt();
}

void Ntupler_monoH_16::findFirstNonElectronMother(const reco::Candidate *particle,
						 int &ancestorPID, int &ancestorStatus){

  if( particle == 0 ){
    printf("Ntupler_monoH_16: ERROR! null candidate pointer, this should never happen\n");
    return;
  }
...
bool
Ntupler_monoH_16::hasWZasMother(const reco::GenParticle  p)  
{
    bool foundZ = false;
...
/*void Ntupler_monoH_16::printCutFlowResult(vid::CutFlowResult &cutflow){
...
DEFINE_FWK_MODULE(Ntupler_monoH_16);
