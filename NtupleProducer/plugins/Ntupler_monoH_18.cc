#include "TagAndProbe_Trigger/NtupleProducer/interface/Ntupler_monoH_18.h"
...
//
Ntupler_monoH_18::Ntupler_monoH_18(const edm::ParameterSet& iConfig):
...
//  tree_->Branch("isTrue"    , &isTrue_);
 ...
 // MonoHiggs
  //tree_->Branch("passFilterDoubleEle33"           ,  &passFilterDoubleEle33);
  tree_->Branch("passFilterDoubleEle33_leg"       ,  &passFilterDoubleEle33_leg);
  tree_->Branch("passFilterDoubleEle27_leg"       ,  &passFilterDoubleEle27_leg);
  tree_->Branch("passFilterDoubleEle25_leg"       ,  &passFilterDoubleEle25_leg);
  tree_->Branch("passFilterEle27_37_leg1"   ,  &passFilterEle27_37_leg1);
  tree_->Branch("passFilterEle27_37_leg2"   ,  &passFilterEle27_37_leg2);
 ...
 // MonoHiggs
  //tree_->Branch("passFilterMu17" ,  &passFilterMu17);
  tree_->Branch("passFilterMu50" ,  &passFilterMu50);
  tree_->Branch("passFilterMu37_TkMu27_leg1" ,  &passFilterMu37_TkMu27_leg1);
  tree_->Branch("passFilterMu37_TkMu27_leg2" ,  &passFilterMu37_TkMu27_leg2);
  ...
  tree_->Branch("passFilterMu27_Ele37_16L1_legMu" ,  &passFilterMu27_Ele37_16L1_legMu);
  tree_->Branch("passFilterMu37_Ele27_16L1_legMu" ,  &passFilterMu37_Ele27_16L1_legMu);
...
Ntupler_monoH_18::~Ntupler_monoH_18()
{
...
void
Ntupler_monoH_18::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace std;
  using namespace edm;
  using namespace reco;
  
  TString ele_filters[13] = {
...
//      ele_isTrue_.clear();
      ...
      // MonoHiggs
      //passFilterDoubleEle33      .clear();
      passFilterDoubleEle33_leg  .clear();
      passFilterDoubleEle27_leg  .clear();
      passFilterDoubleEle25_leg  .clear();
      passFilterEle27_37_leg1  .clear();
      passFilterEle27_37_leg2  .clear();
      ...
      // MonoHiggs
      //bool filterDoubleEle33 = false;
      bool filterDoubleEle33_leg = false;
      bool filterDoubleEle27_leg = false;
      bool filterDoubleEle25_leg = false;
      bool filterEle27_37_leg1 = false;
      bool filterEle27_37_leg2 = false;
...
//
        ...
        // MonoHiggs
        //if(ele_filters[7].Contains(filter) && foundTheLeg)  filterDoubleEle33 = true;
        if(ele_filters[8].Contains(filter) && foundTheLeg)  filterDoubleEle33_leg = true;
        if(ele_filters[9].Contains(filter) && foundTheLeg)  filterDoubleEle27_leg = true;
        cout << "elefilter[9]=" << ele_filters[9] << ", filter=" << filter << ", contains=" << ele_filters[9].Contains(filter) << ", legfound=" << foundTheLeg << endl;
        if(ele_filters[10].Contains(filter) && foundTheLeg)  filterDoubleEle25_leg = true;
        if(ele_filters[11].Contains(filter) && foundTheLeg)  filterEle27_37_leg1 = true;
        if(ele_filters[12].Contains(filter) && foundTheLeg)  filterEle27_37_leg2 = true;
        if(ele_filters[5].Contains(filter) && foundTheLeg)  filterMu27_Ele37_legEle = true;
        if(ele_filters[6].Contains(filter) && foundTheLeg)  filterMu37_Ele27_legEle = true;
      }
      ...
      // MonoHiggs 
      //passFilterDoubleEle33        .push_back(filterDoubleEle33);
      passFilterDoubleEle33_leg    .push_back(filterDoubleEle33_leg);
      passFilterDoubleEle27_leg    .push_back(filterDoubleEle27_leg);
      passFilterDoubleEle25_leg    .push_back(filterDoubleEle25_leg);
      passFilterEle27_37_leg1  .push_back(filterEle27_37_leg1);
      passFilterEle27_37_leg2  .push_back(filterEle27_37_leg2);
...
//  if(isAOD) iEvent.getByToken(muonsToken_,muons);
 ...
 // Handle over L1-muon
  ...
  if(doMuon_)
  {
     ...
     // MonoHiggs
     //passFilterMu17          .clear();
     passFilterMu50          .clear();
     passFilterMu37_TkMu27_leg1 .clear();
     passFilterMu37_TkMu27_leg2 .clear();
     passFilterMu27_Ele37_legMu.clear();
     passFilterMu37_Ele27_legMu.clear();
     passFilterMu27_Ele37_16L1_legMu.clear();
     passFilterMu37_Ele27_16L1_legMu.clear();
     ...
     for(unsigned i=0; i < muons->size();++i ) {
       ...
       // MonoHiggs
       //bool filterMu17 = false;
       bool filterMu50 = false;
       bool filterMu37_TkMu27_Leg1 = false;
       bool filterMu37_TkMu27_Leg2 = false;
       bool filterMu27_Ele37_legMu = false;
       bool filterMu37_Ele27_legMu = false;
       bool filterMu27_Ele37_16L1_legMu = false;
       bool filterMu37_Ele27_16L1_legMu = false;
      ...
      // Trigger matching
       for (unsigned int iteTrigObj = 0 ; iteTrigObj < filterToMatch_.size() ; iteTrigObj++){
          ...
          // MonoHiggs
          //if(mu_filters[6].Contains(filter) && foundTheLeg)  filterMu17 = true;
          if(mu_filters[7].Contains(filter) && foundTheLeg)  filterMu50 = true;
          if(mu_filters[10].Contains(filter) && foundTheLeg)  {filterMu37_TkMu27_Leg1 = true; }
          if(mu_filters[11].Contains(filter) && foundTheLeg)  {filterMu37_TkMu27_Leg2 = true; }
          if(mu_filters[11].Contains(filter) && foundTheLeg && delRL1_16_toBeChecked < 0.3)  {filterMu27_Ele37_16L1_legMu = true; }
          if(mu_filters[8].Contains(filter) && foundTheLeg)  {filterMu27_Ele37_legMu = true; }
          if(mu_filters[10].Contains(filter) && foundTheLeg && delRL1_16_toBeChecked < 0.3) {filterMu37_Ele27_16L1_legMu = true; }
          if(mu_filters[9].Contains(filter) && foundTheLeg) {filterMu37_Ele27_legMu = true; }
       ...
       // MonoHiggs
       //passFilterMu17          .push_back(filterMu17);
       passFilterMu50          .push_back(filterMu50);
       passFilterMu37_TkMu27_leg1 .push_back(filterMu37_TkMu27_Leg1);
       passFilterMu37_TkMu27_leg2 .push_back(filterMu37_TkMu27_Leg2);
       passFilterMu27_Ele37_legMu .push_back(filterMu27_Ele37_legMu);
       passFilterMu37_Ele27_legMu .push_back(filterMu37_Ele27_legMu);
       passFilterMu27_Ele37_16L1_legMu .push_back(filterMu27_Ele37_16L1_legMu);
       passFilterMu37_Ele27_16L1_legMu .push_back(filterMu37_Ele27_16L1_legMu);
...
void 
Ntupler_monoH_18::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
Ntupler_monoH_18::endJob() 
{
}
...
/*
void 
Ntupler_monoH_18::beginRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
Ntupler_monoH_18::endRun(edm::Run const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
Ntupler_monoH_18::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void 
Ntupler_monoH_18::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
void
Ntupler_monoH_18::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
int Ntupler_monoH_18::matchToTruth(const edm::Ptr<reco::GsfElectron> el, 
  ...
  if( ancestorPID == -999 && ancestorStatus == -999 ){
    ...
    // Complain.
    printf("Ntupler_monoH_18: ERROR! Electron does not apper to have a non-electron parent\n");
    return UNMATCHED;
  }
...
bool Ntupler_monoH_18::cmd(const reco::GenParticle & s1, const reco::GenParticle & s2)
{  
   return s1.pt() > s2.pt();
}

void Ntupler_monoH_18::findFirstNonElectronMother(const reco::Candidate *particle,
						 int &ancestorPID, int &ancestorStatus){

  if( particle == 0 ){
    printf("Ntupler_monoH_18: ERROR! null candidate pointer, this should never happen\n");
    return;
  }
...
bool
Ntupler_monoH_18::hasWZasMother(const reco::GenParticle  p)  
{
    bool foundZ = false;
...
/*void Ntupler_monoH_18::printCutFlowResult(vid::CutFlowResult &cutflow){
...
DEFINE_FWK_MODULE(Ntupler_monoH_18);
